// PATH: app/banking-exams/page.tsx
...
export const metadata: Metadata = {
  ...
  alternates: {
    canonical: 'https://prepbanker.com/banking-exams',
  },
};
...
export default function BankingExamsPage() {
  return (
    <>
      ...
      />
      <ItemListSchema
        name="Banking Exams Hub — PrepBanker"
        items={EXAMS.map(e => ({
          name: e.shortName,
      ...
      <div style={{ background:'linear-gradient(135deg,#07102A 0%,#1A2D5A 100%)',padding:'3rem 0 2.5rem' }}>
        <div style={{ maxWidth:1350,margin:'0 auto',padding:'0 1.5rem',textAlign:'center' }}>
          ...
          <p style={{ color:'rgba(255,255,255,0.7)',fontSize:'1rem',maxWidth:600,margin:'0 auto' }}>
            Structured preparation guides for SBI PO and IBPS PO — covering syllabus breakdowns, exam patterns, official dates, historical cut-offs, and mock tests.
          </p>
        </div>
      </div>
        ...
        <div style={{ marginTop:'3rem',background:'#fff',borderRadius:16,border:'1px solid #E2E8F0',padding:'1.5rem 2rem',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:'1rem',boxShadow:'0 2px 20px rgba(13,27,62,0.06)' }}>
          <div>
            ...
            <p style={{ fontSize:'0.875rem',color:'#475569' }}>
              SBI PO offers a higher starting basic pay; IBPS PO gives you recruitment opportunities across 11 public sector banks.
            </p>
          </div>
  ...
  return (
    <div style={{ background:'#fff',borderRadius:18,border:'1px solid #E2E8F0',overflow:'hidden',boxShadow:'0 2px 20px rgba(13,27,62,0.06)',display:'flex',flexDirection:'column',transition:'all 0.25s' }}>
      ...
      <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr 1fr',borderBottom:'1px solid #E2E8F0' }}>
        {[
          { label:'Vacancies', value: exam.id === 'ibps-po' ? '3,500+' : '1,500' },
          { label:'Stages',    value: exam.id === 'ibps-po' ? '3'       : '3'       },
          { label:'Salary',    value: exam.id === 'ibps-po' ? '₹36K+'   : '₹48K+'  },
        ].map(s => (
