          ...
          {quizLimit < CA_QUIZZES.length && (
            <div className="flex justify-center pt-4">
              <button
                ...
                className="px-6 py-2.5 bg-[#FBBF24] hover:bg-[#F59E0B] text-[#07102A] font-extrabold text-[14px] rounded-lg uppercase tracking-wider transition-all shadow-sm hover:shadow-md"
              >
                Load More
              </button>
            </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-grow lg:w-3/4 order-2 lg:order-1 flex flex-col gap-6">
            ...
            <div className="flex items-center justify-between pb-2 border-b border-slate-100">
              <div className="text-[14px] text-[var(--color-gray-400)]">
                Showing{' '}
            ...
            {paginated.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 gap-3 text-center bg-white border border-slate-200 rounded-2xl">
                ...
                <p className="text-[14px] text-gray-450">
                  Try select another category filter.
                </p>
                {filters.search && (
                  <button
                    onClick={clearSearch}
                    className="mt-1 text-[14px] font-semibold text-[var(--color-blue)] hover:underline"
                  >
                    Clear search query
                  </button>
                )}
          ...
          <aside className="w-full lg:w-1/4 lg:max-w-[300px] flex-shrink-0 order-1 lg:order-2 flex flex-col gap-6 lg:sticky lg:top-[160px] self-start">
            ...
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col gap-5">
              ...
              <div>
                <h4 className="text-[14px] font-extrabold text-slate-800 uppercase tracking-wide mb-3">
                  Syllabus Categories
                </h4>
                <div className="space-y-2.5 max-h-[320px] overflow-y-auto pr-1 scrollbar-thin">
                  {[
                    ...
                    return (
                      <label key={cat} className="flex items-center justify-between text-[14px] font-semibold text-slate-700 hover:text-slate-900 cursor-pointer group">
                        ...
                        <span className="text-[14px] text-slate-400 font-bold group-hover:text-slate-600 transition-colors">({count})</span>
                      </label>
                    );
                  })}
                </div>
            ...
            <div className="bg-[#07102A] rounded-xl p-5 text-white relative overflow-hidden border border-slate-800 shadow-md">
              {/* Badge */}
              <div className="absolute top-0 right-0 bg-[#FBBF24] text-[#07102A] text-[14px] font-extrabold px-2.5 py-0.5 rounded-bl uppercase tracking-wide">
                64% OFF
              </div>

              <h4 className="text-[14px] font-extrabold mt-1 pr-10 text-slate-100 uppercase tracking-wide leading-snug">
                Get access to premium subscription
              </h4>

              <div className="mt-4 flex items-baseline gap-2">
                <span className="text-[14px] text-slate-455 line-through">₹549.00</span>
                <span className="text-lg font-extrabold text-[#FBBF24]">₹199/-</span>
              </div>

              <p className="text-[14px] text-slate-300 mt-2 leading-snug">
