  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
      ...
      <nav className="p-2 flex flex-col gap-0.5 max-h-[365px] overflow-y-auto">
        ...
        <Link
          href={`/${exam.id}`}
          className={`flex items-center justify-between px-3 py-2 text-sm sm:text-base font-sans rounded-lg transition-colors border-b border-slate-100 pb-2.5 mb-1 col-span-full ${
            !activeSlug
        ...
        {sidebarLinks.map((link) => {
          ...
          return (
            <Link
              key={link.slug}
              href={href}
              className={`flex items-center justify-between px-3 py-1.5 text-sm transition-colors rounded-lg font-sans ${
                isActive
                  ? 'font-black text-[#1B6EB5] bg-slate-100/80 border-l-4 border-[#1B6EB5] rounded-r-lg'
                  : 'font-bold text-[#1B6EB5] hover:bg-slate-50 hover:underline'
              }`}
